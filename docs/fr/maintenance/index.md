---
layout: default
title: Maintenance
parent: YourSqlDba documentation
nav_order: 40
has_children: true
---

# Maintenance

La maintenance régulière de YourSqlDba est pilotée par des tâches SQL Agent qui appellent
`Maint.YourSqlDba_DoMaint`.

Cette section documente comment les DBA utilisent et personnalisent cette procédure.

- [Maint.YourSqlDba_DoMaint](your-sql-dba-domaint.md)
- [Miroir, standby et tests de migration](mirror-standby-migration.md)
- [Délégation de la gestion de base de données](delegated-database-management.md)
- [Solutions de sauvegarde externes](external-backup-solutions.md)

