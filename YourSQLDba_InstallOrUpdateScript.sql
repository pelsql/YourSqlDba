...
===KeyWords===*/
GO
Drop Function If Exists S#.GetCmtBetweenDelim
GO
Create Function [S#].[GetCmtBetweenDelim] (@delim sysname, @ModuleRef Sql_Variant)
Returns table
as
-----------------------------------------------------------------------------------------------
...
--  /*===DropAllFct===
--  Drop Function if exixts #FctName#
--  ===DropAllFct===*/
...
--go
--Create Function dbo.ScriptFunctionDrops ()
--returns table
--as
--return
--Select Sql
...
--go
--Create Proc dbo.DropsFunctions
--as
--Begin
--  Declare @Sql Nvarchar(max) = ''
--  Select @sql=@sql+Sql
...
GO
Drop function if exists S#.ConcatFromJson
GO
Create Function S#.ConcatFromJson(@InitSeparator nvarchar(10), @Separator nvarchar(10), @json nvarchar(max)) -- makes concat easier to do
Returns Table
As
Return
-----------------------------------------------------------------------------------------------------
...
GO
Drop function if exists S#.MultipleReplaces
GO
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
Create Function S#.MultipleReplaces (@Template nvarchar(max), @JsonDataSource Nvarchar(max))
Returns Table
as
Return 
---
...
-------------------------------------------------------------------------------------
Drop function If exists S#.GetTemplateFromCmtAndReplaceTags
GO
Create Function S#.GetTemplateFromCmtAndReplaceTags (@delim sysname, @CmtSource Sql_Variant, @JSonDataSource Nvarchar(max))
Returns table
As
Return
...
-- -------------------------------------------------------------------------------------------------------------
Drop Function If Exists S#.FullObjName
GO
create function S#.FullObjName
( 
@object_id Int
)
Returns sysname
...
-- -------------------------------------------------------------------------------------------
Drop Proc If Exists S#.PrintSplittedSqlCode
GO
Create Procedure S#.PrintSplittedSqlCode 
  @Sql Nvarchar(Max)
, @Label Nvarchar(Max) = Null
, @NextBatch Int = 1
As
...
------------------------------------------------------------------------
Drop Function If Exists S#.FormatRunTimeMsg
GO
Create Function S#.FormatRunTimeMsg 
(
...
--------------------------------------------------------------------------------------------------------
Drop Proc If Exists S#.PerformCheckDbAndReportErrorsIfAny
GO
Create Procedure S#.PerformCheckDbAndReportErrorsIfAny @Db Sysname
As
Begin
  Set nocount on
...
-- -------------------------------------------------------------------------------------------
GO
Drop Function If Exists S#.GetErrorMessagesAndInfo
GO
Create Function S#.GetErrorMessagesAndInfo()
Returns Table
as
Return
Select *
...
===KeyWords===*/
Go
Drop Procedure If Exists S#.ModifyShowLastRunScriptQry 
GO
------------------------------------------------------------------------------
-- This procedure is altered at every query run by RunScript
-- It conveniently displays last running query attempted to be run
------------------------------------------------------------------------------
Create Procedure S#.ModifyShowLastRunScriptQry @Sql Nvarchar(max)
as
Begin
...
---------------------------------------------------------------------------------------------------------
Drop Function If Exists S#.DedupSeqOfChar
GO
Create Function S#.DedupSeqOfChar (@Dup nvarchar(5), @Str Nvarchar(max))
Returns table
As
Return
(  
...
GO
Drop View If exists dbo.WhoCalledWhat
GO
Create View dbo.WhoCalledWhat
As
-- ---------------------------------------------------------------------------------------------
...
from 
  ...
  /*
  drop proc if exists a
  go
  create proc A as Exec('B')
  go
  drop proc if exists B
  go
  create proc B as Select * from dbo.WhoCalledWhat
  go
  Exec A
  */
GO
Drop function If exists dbo.ScriptSetGlobalAccessToPrm 
GO
Create Function dbo.ScriptSetGlobalAccessToPrm (@PrmSetInJson as Nvarchar(max))
returns table
-- =============================================================================================================
--
...
-- -------------------------------------------------------------------------------------------------------------
Drop Function If Exists Dbo.InferObjectNamings
GO
Create Function Dbo.InferObjectNamings(@objRef SQL_Variant, @DbId Int) 
Returns Table
as
Return
(
...
GO
Drop Function If EXISTS Dbo.DbsFileSizes
GO
Create Function Dbo.DbsFileSizes (@DbNameLike Sysname)
-- ----------------------------------------------------------------------------------------------------------
...
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yInstall.SqlVersionNumber
go
...
go
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yUtl.ReduceRepeatingSpaceToOne
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists PerfMon.GetBestPracticesMsgs
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
...
GO
Drop Procedure If Exists PerfMon.ReportIgnoredBestPractices
GO
...
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yExecNLog.QryReplace
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
...
GO
Drop Function If Exists yUtl.ColumnInfo
go
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yExecNLog.FormatBasicBeginCatchErrMsg
GO
...
go
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yExecNLog.Unindent_TSQL -- newer shema
GO
-- ------------------------------------------------------------------------------
...
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yExecNLog.ReplaceByXmlEscapeChar
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
...
go
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yInstall.DoubleLastSpaceInFirst78Colums
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yInstall.DoubleLastSpaceInFirst150Colums
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop View If exists PerfMon.LockChainView
GO
...
------------------------------------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists PerfMon.ResetAnalyzeWaitStats
GO
...
go
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists PerfMon.AnalyzeWaitStats
GO
...
go
Drop Function If Exists yPerfMon.ActiveQueryInBatch
go
...
go
Drop View If Exists perfmon.SessionInfo
go
...
Drop Function If Exists PerfMon.DetailQueriesStats
go
...
End
GO
Drop Function If Exists dbo.MainContextInfo
GO
Create Function dbo.MainContextInfo (@jobNo Int)
returns table
as
return
-- ---------------------------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------------------------
drop FUNCTION if exists yExecNLog.SqlCodeLinesInResultSet 
GO
...
go
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Install.PrintVersionInfo
GO
...
go
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yInstall.NextUpdateTime
GO
...
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yUtl.UnicodeCrLf
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
Drop Function If Exists yUtl.NormalizeLineEnds
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yUtl.YourSQLDba_ApplyFilterDb
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yUtl.NormalizePath
GO
-- ------------------------------------------------------------------------------
...
-----------------------------
Drop Function If Exists yUtl.SearchWord
GO
-- Alter 
...
Drop Function if Exists  yUtl.SearchWords
GO
-- Alter 
...
If Object_id('S#.LogTable') IS NULL Exec ('Create Function S#.LogTable() Returns sysname as Begin Return('''') End')
GO
-- -------------------------------------------------------------------------------------------
-- Help printing out SQL Code, by dividing lines at cr/lf
-- Workaround for SQL Print output limited to 8000 chars in SQL Management Studio 
...
-- -------------------------------------------------------------------------------------------
Drop Procedure If Exists S#.RegisterLogTable
GO
Create Procedure S#.RegisterLogTable @LogTable sysname = NULL
As
Begin
  Set Nocount On
  ...
  From 
    ...
    Create function S#.LogTable () Returns sysname as Begin Return('#logTable#') End
    ===CreateLogTableFunction===*/
  Exec (@Sql)
...
End
GO
Drop Proc If Exists S#.QueryLog
go
Create Procedure S#.QueryLog @like nvarchar(max) = NULL, @NomLog sysname = NULL
as
Begin
  Set Nocount On
...
-- -------------------------------------------------------------------------------------------
Drop Proc If Exists S#.PrintAndLog
GO
Create Procedure S#.PrintAndLog
  @LogText Nvarchar(Max) -- text to log
, @nextBatch Int = 0 -- 
As
Begin
...
Drop Procedure If Exists yExecNLog.ExecWithProfilerTrace
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
-- ------------------------------------------------------------------------------
...
Create Proc dbo.LogForDebugging @DebugMessage Nvarchar(max)
as
Begin
...
End
GO
Drop Procedure If Exists yMaint.CollectBackupHeaderInfoFromBackupFile
go
...
End
Go
Drop Procedure If Exists yMaint.CollectBackupFileListFromBackupFile
go
...
Drop Procedure If Exists yMaint.SaveXpCmdShellStateAndAllowItTemporary
GO
...
End
GO
Drop Procedure If Exists yMaint.RestoreXpCmdShellState
GO
...
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yMaint.PutDbOffline
GO
...
-- ---------------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Maint.DiagDbMail
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yUtl.ConvertToHexString
GO
...
----------------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Tools.CompareRows
GO
-------------------------------------------------------------------------------------
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Audit.GenerateIt
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Audit.SuspendIt
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Audit.ReactivateIt
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Audit.RemoveIt
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Audit.ProcessExpiredDataAudits
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Audit.ProcessDataAuditsCleanup
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Audit.ProcessDataAuditsCleanupForAllDb
GO
...
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Maint.ShowJobErrors
GO
-- this procedure allows calling HistoryView from SqlAgent history
create proc Maint.ShowJobErrors 
  @JobNo Int
as
Begin
  Select 
...
-- ------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yMaint.CheckFullRecoveryModelPolicy
GO
...
-- ------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yMaint.ShrinkLog
GO
...
-- ------------------------------------------------------------------------------
Drop Proc If Exists Maint.ShrinkAllLogs
GO
Create Proc Maint.ShrinkAllLogs
as
Begin
  Declare @Sql Nvarchar(max)
  Declare @ignore int
...
-- ------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Maint.BringBackOnlineAllOfflineDb
GO
...
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yMaint.LogCleanup
GO
...
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yMaint.IntegrityTesting
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
----------------------------------------------------------------------------------------
...
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yMaint.UpdateStats
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yMaint.ReorganizeOnlyWhatNeedToBe
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
...
-- ------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yInstall.InstallationLanguage
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
...
-- ------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yMaint.MakeBackupFileName
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
...
-- ------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yMaint.MakeBackupCmd
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
...
-- ------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yMirroring.Broker_AutoActivated_LaunchRestoreToMirrorCmd
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
...
-- ---------------------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yMirroring.QueueRestoreToMirrorCmd
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
...
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Maint.DropOrphanLogins
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
...
-- ------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Maint.DeleteOldBackups
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yMirroring.MirrorLoginSync
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yMirroring.LaunchLoginSync
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
go
Drop Procedure If Exists yMirroring.ReportYourSqlDbaVersionOnTargetServers
GO
...
-- ------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yMaint.Backups
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
Drop Procedure If Exists Mirroring.SetYourSqlDbaAccountForMirroring
GO
...
----------------------------------------------------------------------------------------------------------
Drop Procedure If Exists yMirroring.CleanMirrorServerForMissingServerAndCheckServerAccessAsYourSqlDbaAccount
GO
...
Go
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yUtl.DedupSeqOfChars
GO
Create Function yUtl.DedupSeqOfChars (@Dup nvarchar(5), @Str Nvarchar(max))
Returns Table
as
Return
(
...
-------------------------------------------------------------------------------------------
Drop Proc If Exists Maint.SetSyncWith_YourSqlDba_DoMaint
GO
Create Proc Maint.SetSyncWith_YourSqlDba_DoMaint @WaitType Sysname = 'Exclusive'
AS
Begin
...
End
GO
Drop Proc If Exists Maint.SignalEndOf_YourSqlDba_DoMaint 
GO
Create Proc Maint.SignalEndOf_YourSqlDba_DoMaint 
AS
...
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Maint.YourSqlDba_DoMaint
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
Drop proc If Exists Maint.YourSqlDba_DoMaint
GO
...
-- ------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yUtl.GetPathFromName
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
...
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Maint.SaveDbOnNewFileSet
GO
-- -------------------------------------------------------------------------------------------------------------
...
GO
Drop Procedure If Exists Maint.SaveDbCopyOnly
GO
...
Drop Procedure If Exists Maint.duplicateDb
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Maint.duplicateDbFromBackupHistory
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Maint.RestoreDb
GO
...
-- ------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yUtl.TextSplitInRows
GO
...
-- ---------------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Maint.ShowHistory
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
...
---------------------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Install.AddOrReplaceMaintenance
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
...
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Install.InitialSetupOfYourSqlDba
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
...
Drop Procedure If Exists yInstall.CleanUpParam
GO
...
Drop Procedure If Exists yInstall.AddUpEndParam
GO
...
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yInstall.ReplaceParamValue
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Function If Exists yInstall.ReplaceRetDays
GO
-- This function replace the parameters name in the "@command" string
-- and subtract 1 day from the number of days of retention.
...
Drop Procedure If Exists Install.UpdateMaintenanceTasksParam
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yInstall.DoReplacesInJobAndTasks
GO
...
Drop Procedure If Exists Maint.CreateNetworkDrives
GO
...
Drop Procedure If Exists Maint.DisconnectNetworkDrive
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
Drop Procedure If Exists Maint.ListNetworkDrives
GO
...
Drop Procedure If Exists Mirroring.DropServer
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
Drop Procedure If Exists Mirroring.AddServer
GO
...
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yMirroring.DoRestore
GO
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
...
-- -------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Maint.PrepDbForMaintenanceMode
GO
...
-- -------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yMaint.PrepareRestoreDbToLogMarkCommand
GO
...
-- -------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Maint.RestoreDbAtStartOfMaintenanceMode
GO
...
-- -------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Maint.ReturnDbToNormalUseFromMaintenanceMode
GO
...
drop proc if exists S#.CopyAllSqlModules
go
Create Proc S#.CopyAllSqlModules @SrcDb Sysname, @DestDb Sysname
as
Begin
  Set nocount on
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yMirroring.DatabaseRecovery
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Mirroring.DoRecovery
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists yUpgrade.UpgradeFulltextCatalogsFromSql2005
GO
...
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Upgrade.MakeDbCompatibleToTarget
GO
...
--------------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
Drop Procedure If Exists Mirroring.Failover
GO
------------------------------------------------------------------------------------------
If Db_name() <> 'YourSqlDba' Use YourSqlDba
GO
